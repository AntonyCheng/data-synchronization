...
- **raw topic 的内容与保留**：raw topic 与目标 topic 在同一个 Kafka 集群，保存的是整行（包括未选中的字段，§1）。未选字段中有不应出现在该集群上的数据时，需在 broker 上限制 `__ds_raw_*` 的访问权限。平台以 1 分区 / 1 副本创建 raw topic，不设置主题级参数，保留期跟随 broker 默认（`log.retention.hours`，默认 168 小时），磁盘占用约为“保留期内的变更行数 × 单条体积”。平台删除不再使用的 raw topic：
  ...
  - **宽限期与删除**：首次发现不再使用时只记下时间，满 `sync.kafka-bridge.raw-topic-retire-grace`（默认 `10m`）后才删除。配置版本只在作业停止时才会变，但其他实例上的桥接 worker 要等各自的对账（30 秒）才退出，宽限期留足余量。owner 已被删除时，其 consumer group `ds-task-{id}-bridge` 随 topic 一起删除（group 不存在或仍有成员不算错误）；owner 还在时该 group 要继续读新 topic，保留。删除后登记再保留一个宽限期：期间 topic 若被残留的客户端重新建出来就再删一次，之后删除登记。桥接自己的 consumer 设置了 `allow.auto.create.topics=false`，不会重建 raw topic。
