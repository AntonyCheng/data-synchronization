...
| `KafkaTaskE2eTest` | FULL_CDC→Kafka（ENVELOPE） | 经平台接口建输出 topic；消费真实输出 topic：初始装载 INSERT（`phase=SNAPSHOT`，恰为 3 条且全部先于第一条 `CDC` 事件）、CDC INSERT（`sourceEventTime` 为整秒的 binlog 事件时间）、单条 UPDATE（带 before 前像）、DELETE（data 为被删行），消息 Key 为同步键 JSON；停止后引擎无作业 |
...
MySQL `FULL_CDC`→Kafka 的初始装载事件 `phase` 是 `SNAPSHOT`：引擎以 `compatible_debezium_json` 写 raw topic，初始装载行是 Debezium 的 `op=r`（`kafka-event-formats.md` 的“phase 的含义”与 §9）。`KafkaTaskE2eTest` 按此断言，所以它要求被测后端运行的是包含这一变更的代码；对旧后端运行会在第一条初始装载事件上失败（旧后端发布的是 `CDC`）。
