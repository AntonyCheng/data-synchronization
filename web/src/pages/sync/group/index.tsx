...
const emptyForm: SyncTaskGroupForm = {
...
export default function SyncTaskGroupPage() {
  ...
  const openAdd = () => {
    ...
    setItemMetadata({});
    setTargetTables([]);
    setModalTitle('新增多表同步任务');
    setModalOpen(true);
  };
  const openEdit = async (row: SyncTaskGroupVO) => {
    const result = await getSyncTaskGroup(row.groupId);
    form.resetFields();
    ...
    setModalTitle('修改多表同步任务');
    setModalOpen(true);
  };
  const openDetail = async (row: SyncTaskGroupVO) => {
    const result = await getSyncTaskGroup(row.groupId);
  ...
  const validate = async (row: SyncTaskGroupVO) => {
    const result = await validateSyncTaskGroup(row.groupId);
    setValidationResult(result.data);
  };
  const preview = async (row: SyncTaskGroupVO) => {
    const result = await previewSyncTaskGroupConfig(row.groupId);
  ...
  const discover = async (row: SyncTaskGroupVO) => {
    const result = await discoverSyncTaskGroupTables(row.groupId);
    message.success(result.data.message);
    actionRef.current?.reload();
  };
  ...
  const reinitializeItem = (row: SyncTaskGroupVO, itemId: string | number, sourceTable?: string) =>
    Modal.confirm({
        ...
        <Space size={6} wrap={false} className="group-row-actions-react">
          ...
          {can('sync:group:remove') && (
            <Button
              ...
              danger
              icon={<DeleteOutlined />}
              disabled={['RUNNING', 'PAUSING', 'DEGRADED'].includes(row.status || '')}
              title={
                ['RUNNING', 'PAUSING', 'DEGRADED'].includes(row.status || '') ? '运行中的任务组不能删除' : undefined
              }
              onClick={() =>
                Modal.confirm({
                  title: '删除同步任务组',
            ...
            {({ sourceId }) => (
              <ProFormSelect
                ...
                fieldProps={{
                  style: { width: 300 },
                  onChange: value => {
                    const target = dataSources.find(item => String(item.sourceId) === String(value));
                    const items = form.getFieldValue('items') || [];
                    form.setFieldsValue({
                      items: items.map((item: NonNullable<SyncTaskGroupForm['items']>[number]) => ({
                        ...item,
                        targetSchema: target?.sourceType === 'POSTGRESQL' ? item.targetSchema || 'public' : undefined
                      }))
                    });
                    setTargetTables([]);
                    ...
                    <div key={field.key} style={{ borderBottom: '1px solid #f0f0f0', paddingBottom: 12 }}>
                        ...
                        >
                          {kafkaTarget ? (
                            <AutoComplete
                              ...
                              allowClear
                              placeholder={
                                targetTablesLoading ? '正在读取 topic 列表' : '选择已有 topic 或输入新 topic'
                              }
                              style={{ width: 220 }}
                            />
                          ) : (
                            <AutoComplete
                              ...
                              allowClear
                              placeholder={
                                targetTablesLoading ? '正在读取目标表，可直接输入新表名' : '选择已有表或输入新表名'
                              }
                              style={{ width: 220 }}
                            />
                          )}
                      ...
                      {kafkaTarget && (
                        <Alert
                          type="info"
                          showIcon
                          message="可选择已有 topic，也可输入名称后点击“创建 topic”；平台不会依赖 broker 自动建 topic。"
                          style={{ marginTop: 8 }}
                        />
                      )}
                      {!kafkaTarget && (
                        <Alert
                          type="info"
                          showIcon
                          message="可选择目标库已有表，也可直接输入新表名；新表启动时按源表字段结构自动创建。"
                          style={{ marginTop: 8 }}
                        />
                      )}
                      {metadata && (
          ...
          <Space direction="vertical" size={16} style={{ width: '100%' }}>
            ...
            <Space wrap size={8}>
              ...
              {can('sync:group:status') && (
                <Button icon={<ReloadOutlined />} onClick={() => operate(detail, 'status')}>
                  刷新状态
                </Button>
              )}
              {can('sync:group:pause') && (
                <Button
                  ...
                  disabled={!['RUNNING', 'DEGRADED'].includes(detail.status || '')}
                  onClick={() => operate(detail, 'pause')}
                >
                  暂停
                </Button>
              )}
              {can('sync:group:resume') && (
                <Button
                  icon={<PlayCircleOutlined />}
                  disabled={!['PAUSED', 'FAILED'].includes(detail.status || '')}
                  onClick={() => operate(detail, 'resume')}
                >
                  恢复
                </Button>
              )}
              {can('sync:group:stop') && (
                <Button
                  danger
                  icon={<StopOutlined />}
                  disabled={!['RUNNING', 'PAUSING', 'PAUSED', 'FAILED', 'DEGRADED'].includes(detail.status || '')}
                  onClick={() =>
                    Modal.confirm({
                      title: '中止同步任务组',
            ...
            <Space wrap size={8}>
              ...
              {can('sync:group:discover') && detail.syncScope === 'DATABASE' && (
                <Button icon={<ReloadOutlined />} onClick={() => discover(detail)}>
                  扫描新表
                </Button>
              )}
              {can('sync:group:engine-config') && (
                <Button icon={<EyeOutlined />} onClick={() => preview(detail)}>
                  配置预览
                </Button>
              )}
              {can('sync:group:validate') && (
                <Button icon={<SafetyCertificateOutlined />} onClick={() => validate(detail)}>
                  连接与配置校验
                </Button>
              )}
              {can('sync:group:ddl-check') && (
                <Button icon={<SafetyCertificateOutlined />} onClick={() => ddlCheck(detail)}>
                  结构检查
                </Button>
              )}
            </Space>
                            ...
                            <Space wrap size={8}>
                              ...
                              {can('sync:group:reinitialize') && (
                                <Button
                                  size="small"
                                  danger
                                  onClick={() => reinitializeItem(detail, event.itemId, event.sourceTable)}
                                >
                                  重新初始化该表
                                </Button>
                              )}
            ...
            <Button
              ...
              disabled={['RUNNING', 'PAUSING', 'DEGRADED'].includes(detail.status || '')}
              title={
                ['RUNNING', 'PAUSING', 'DEGRADED'].includes(detail.status || '') ? '运行中的任务组不能删除' : undefined
              }
              onClick={() =>
                Modal.confirm({
                  title: '删除同步任务组',
            ...
            <Descriptions bordered size="small" column={1} title="表项状态">
              {detail.items.map(item => (
                <Descriptions.Item
                  ...
                  label={`${item.sourceDatabase || '-'} . ${item.sourceTable} -> ${detailTarget?.sourceType === 'POSTGRESQL' ? `${item.targetSchema || 'public'} . ` : ''}${item.targetTable}`}
                >
                  <Space wrap size={8}>
                    ...
                    {can('sync:group:reinitialize') &&
                      ...
                      !['DRAFT', 'PAUSING'].includes(detail.status || '') && (
                        <Button
                          size="small"
                          danger
                          onClick={() => reinitializeItem(detail, item.itemId, item.sourceTable)}
                        >
                          重新初始化该表
                        </Button>
                      )}
