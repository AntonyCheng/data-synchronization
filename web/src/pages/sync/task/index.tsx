...
const defaultForm: SyncTaskForm = { syncMode: 'FULL_CDC', incrementalStartupMode: 'LATEST', fullDataMode: 'UPSERT', ddlPolicy: 'FAIL', scheduleMode: 'ONCE', targetSchema: 'public', kafkaOutputFormat: 'ENVELOPE', readLimitRowsPerSecond: 1000, readLimitBytesPerSecond: 10485760, snapshotParallelism: 1, sourceConnectionLimit: 2 };
...
export default function SyncTaskPage() {
  ...
  const submitForm = async (values: SyncTaskForm) => {
    ...
    const payload = {
      ...
      targetSchema: target?.sourceType === 'POSTGRESQL' ? allValues.targetSchema : undefined,
      kafkaOutputFormat: target?.sourceType === 'KAFKA' ? allValues.kafkaOutputFormat : undefined
    };
        ...
        {selectedTask && (
          <Space direction="vertical" size={18} style={{ width: '100%' }}>
            <Descriptions bordered size="small" column={{ xs: 1, sm: 2, md: 3 }}>
              ...
              {selectedTargetIsKafka && <Descriptions.Item label="Kafka 输出格式">{kafkaOutputFormatLabel(selectedTask.kafkaOutputFormat)}</Descriptions.Item>}
