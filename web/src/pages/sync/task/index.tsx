...
const defaultForm: SyncTaskForm = {
  syncMode: 'FULL_CDC',
...
const kafkaOutputFormatLabel = (value?: string) =>
...
export default function SyncTaskPage() {
  ...
  const checkData = async () => {
    ...
    setBusyAction('check');
    try {
      const result = await checkSyncTaskData(selectedTask.taskId, {
        mode: checkMode,
  ...
  const targetOptions = dataSources
    ...
    .map(item => ({ label: sourceLabel(item), value: item.sourceId }));
  const selectedTargetType = dataSources.find(
    item => String(item.sourceId) === String(selectedTask?.targetId)
  )?.sourceType;
  ...
  const columns: ProColumns<SyncTaskVO>[] = [
    ...
    { title: '更新时间', dataIndex: 'updateTime', width: 170, search: false },
    {
      ...
      render: (_, row) => {
        const canAbort =
          canStop && ['RUNNING', 'PAUSING', 'PAUSED'].includes(row.status || '') && Boolean(row.engineJobId);
          ...
          <Space direction="vertical" size={18} style={{ width: '100%' }}>
            <Descriptions bordered size="small" column={{ xs: 1, sm: 2, md: 3 }}>
              <Descriptions.Item label="任务状态">
                {statusTag(selectedTask.status, selectedTask.lastError)}
              </Descriptions.Item>
              <Descriptions.Item label="同步模式">
                {selectedTask.syncMode === 'FULL_CDC' ? '全量 + CDC' : selectedTask.syncMode || '-'}
              </Descriptions.Item>
              <Descriptions.Item label="调度模式">
                {selectedTask.scheduleMode === 'CRON'
                  ? `定时 Cron${selectedTask.cronExpression ? ` (${selectedTask.cronExpression})` : ''}`
                  : selectedTask.scheduleMode === 'REALTIME'
                    ? '常驻实时'
              ...
              <Descriptions.Item label="目标表">
                {selectedTargetIsPostgres
                  ? `${selectedTask.targetSchema || 'public'}.${selectedTask.targetTable || '-'}`
                  : selectedTask.targetTable || '-'}
              </Descriptions.Item>
              <Descriptions.Item label="最近检查点">{selectedTask.lastCheckpointTime || '-'}</Descriptions.Item>
              <Descriptions.Item label="最近核对">
                {selectedTask.lastCheckTime
                  ? `${selectedTask.lastCheckTime} / ${selectedTask.lastCheckMatched === '1' ? '一致' : '不一致'}`
                  : '未核对'}
              </Descriptions.Item>
              {selectedTargetIsKafka && (
                <Descriptions.Item label="Kafka 输出格式">
                  {kafkaOutputFormatLabel(selectedTask.kafkaOutputFormat)}
                </Descriptions.Item>
              )}
              {selectedTargetIsKafka && (
                <Descriptions.Item label="Kafka 已发布事件">
                  {selectedTask.kafkaPublishedCount ?? '-'}
                </Descriptions.Item>
              )}
              {selectedTargetIsKafka && (
                <Descriptions.Item label="Kafka 最近分区/offset">
                  {selectedTask.kafkaLastPartition == null
                    ? '-'
                    : `${selectedTask.kafkaLastPartition} / ${selectedTask.kafkaLastOffset ?? '-'}`}
                </Descriptions.Item>
              )}
              {selectedTargetIsKafka && (
                <Descriptions.Item label="Kafka 最近源事件时间">
                  {selectedTask.kafkaLastSourceEventTime || '-'}
                </Descriptions.Item>
              )}
              {selectedTargetIsKafka && (
                <Descriptions.Item label="Kafka 最近 broker 确认">
                  {selectedTask.kafkaLastBrokerAckTime || '-'}
                </Descriptions.Item>
              )}
              {selectedTargetIsKafka && (
                <Descriptions.Item label="Kafka 发布延迟">
                  {selectedTask.kafkaLagSeconds == null ? '-' : `${selectedTask.kafkaLagSeconds} 秒`}
                </Descriptions.Item>
              )}
            ...
            <MetricsHistoryPanel
              load={minutes => getSyncTaskMetrics(selectedTask.taskId, minutes).then(result => result.data)}
            />
            {selectedTask.lastError && (
              <Alert
                ...
                showIcon
                message="最近一次错误"
                description={<Typography.Text copyable>{selectedTask.lastError}</Typography.Text>}
              />
            )}
            {selectedTask.lastSkipReason && (
              <Alert type="warning" showIcon message="最近一次调度未执行" description={selectedTask.lastSkipReason} />
            )}
            <Divider titlePlacement="left" plain>
              准备与诊断
            </Divider>
            <Space wrap>
              {canCdcPrecheck && selectedTask.syncMode === 'FULL_CDC' && (
                <Button
              ...
              {canStatus && selectedTask.engineJobId && (
                <Button
                  icon={<ReloadOutlined />}
                    ...
                    <Descriptions size="small" column={1}>
                      ...
                      <Descriptions.Item label="水位提示">
                        {checkResult.watermarkMessage ||
                          (selectedTask.syncMode === 'FULL_CDC'
            ...
            <Space wrap>
              {canStart && ['DRAFT', 'STOPPED', 'FINISHED'].includes(taskStatus) && (
                <Button
                  type="primary"
              ...
              {canResume && ['PAUSED', 'FAILED'].includes(taskStatus) && (
                <Button
                  type="primary"
                  icon={<PlayCircleOutlined />}
              ...
              {canReinitialize && ['REINITIALIZE_REQUIRED', 'FAILED'].includes(taskStatus) && (
                <Button
                  type="primary"
                  danger
                  icon={<ReloadOutlined />}
              ...
              {canRemove && (
                <Button
                  ...
                  onClick={() =>
                    Modal.confirm({
                      title: '删除同步任务',
                      content: `是否确认删除任务“${selectedTask.taskName}”？`,
                      okText: '确认删除',
                      cancelText: '取消',
                      onOk: () => remove(selectedTask)
                    })
                  }
                >
                  删除任务
            ...
            <Space>
              ...
              {wizardStep < 4 ? (
                ...
                <Button type="primary" loading={submitLoading} onClick={() => form.submit()}>
                  {form.getFieldValue('taskId') ? '确认保存' : '确认创建'}
                </Button>
              )}
            </Space>
          )
        ...
        {wizardStep === 2 && (
          <>
            <Form.Item noStyle shouldUpdate={(prev, current) => prev.targetId !== current.targetId}>
              {({ getFieldValue }) =>
                getFieldValue('targetId') &&
                dataSources.find(item => String(item.sourceId) === String(getFieldValue('targetId')))?.sourceType ===
                  'KAFKA' ? (
                  <>
                  ...
                  <>
                    ...
                    <Alert
                      type="info"
                      showIcon
                      message="关系型目标已有表会执行兼容性检查；新表在任务启动时自动创建。"
                    />
                  </>
                )
              }
          ...
          <>
            ...
            <Form.Item noStyle shouldUpdate>
              {({ getFieldsValue }) => {
                ...
                const isTargetPostgres =
                  dataSources.find(item => String(item.sourceId) === String(values.targetId))?.sourceType ===
                  'POSTGRESQL';
                const isTargetKafka =
                  dataSources.find(item => String(item.sourceId) === String(values.targetId))?.sourceType === 'KAFKA';
                return (
                  <Descriptions size="small" column={2} bordered>
                    ...
                    <Descriptions.Item label="目标端">
                      {isTargetPostgres ? `${values.targetSchema || 'public'} / ` : ''}
                      {values.targetTable || '-'}
                    </Descriptions.Item>
