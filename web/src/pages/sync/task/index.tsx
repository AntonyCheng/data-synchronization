        ...
        {selectedTask && (
          <Space direction="vertical" size={18} style={{ width: '100%' }}>
            ...
            <Space wrap>
              ...
              {canReinitialize && taskStatus === 'REINITIALIZE_REQUIRED' && <Button type="primary" danger icon={<ReloadOutlined />} loading={busyAction === 'reinitialize'} disabled={actionDisabled} onClick={() => Modal.confirm({ title: '重新初始化同步任务', content: selectedTask.syncMode === 'FULL_CDC' ? '此操作会丢弃旧 checkpoint/savepoint，重新执行全量并继续 CDC。目标端将按当前全量数据模式处理，是否继续？' : '此操作会丢弃旧恢复状态并重新执行全量，是否继续？', okText: '确认重新初始化', cancelText: '取消', onOk: () => runAction('reinitialize', reinitializeSyncTask) })}>重新初始化</Button>}
