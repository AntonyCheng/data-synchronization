...
export function checkSyncTaskGroupDdl(id: string | number) {
  return request<R<SyncTaskGroupDdlCheckResult>>({ url: `/sync/group/${id}/ddl-check`, method: 'post' });
}
export function checkSyncTaskGroupData(id: string | number) {
...
export function getSyncTaskGroupItemMetrics(groupId: string | number, itemId: string | number, minutes: number) {
  return request<R<SyncMetricsSeries>>({
    url: `/sync/group/${groupId}/item/${itemId}/metrics`,
    method: 'get',
    params: { minutes }
  });
}
