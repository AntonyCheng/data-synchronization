...
此模板提供 PRD v0.4 MVP 的离网部署骨架：MySQL 元数据库、Redis、SeaTunnel、Kafka、前端静态站点和本机 Java 21 后端。源端 MySQL 与目标 PostgreSQL/MySQL/Kafka 是被管理的数据源，不会作为业务数据源容器打入本包。

## 启动

1. 在有网构建机执行 `deploy/build-offline-package.ps1`，获得完整目录和 `images/data-sync-mvp-images.tar`。
...
6. 运行 `./start-backend.ps1 -Background`。脚本会先执行包内 15 份幂等结构迁移（含 Kafka 指标字段），再启动 Java 21 后端。
